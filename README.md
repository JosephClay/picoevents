...
* Only **97 bytes** (minified and brotlied).
...
```js
import { e as eventemitter } from 'picoevents'
...
```sh
npm install picoevents
```


## TypeScript

Pico Events accepts interface with event name
...
Because Pico Events API has only just 2 methods,
...
```ts
import { eventemitter, Emitter } from "picoevents"
...
class Ticker {
  ...
  constructor () {
    this.emitter = eventemitter<Events>()
  }
...
Method `emit` will execute all listeners. First argument is event name, second
